...
- **Charts and comparisons**, in two kinds, and the distinction is the point:
  ...
  - Still open, in rough priority: a wage distribution ("you earn more than N % of workers"), which needs a percentile series confirmed at INE/GEP — INE's API sends no CORS header, so it would ship bundled rather than fetched. A standalone market **dashboard** is deliberately *not* planned: every chart so far explains the user's own number, and a data-publication surface is a different product with a recurring editorial burden.
